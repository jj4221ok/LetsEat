...
public class CService {
		...
		if (result != null) {
			if (pwEnc.matches(cPw, encPw)) {
				if (result.getcKey() == 1) {
					...
					session.setAttribute("cloginPhone", result.getcPhone());
	...
	public ModelAndView cModify(CompanyDTO company) {
		...
		if (result > 0) {
			mav.setViewName("redirect:cview?cId=" + company.getcId());
		} else {
	...
	public String sendcEmail(String cId, String cEmail) {
		...
		String str = "<h2>안녕하세요.  Let's Eat 입니다.</h2><br>" + "<h3>" + cId + "님</h3>"
				+ "<h3>점주님의 인증번호는 <span style='color:#ff0000'>" + code + "</span> 입니다.</h3>"
				+ "<p>비밀번호 찾기 페이지로 돌아가서 인증번호를 입력하고 인증을 마무리해주시기 바랍니다.</p><br><br>" + "(본인이 인증요청을 하지 않았을 경우 무시하셔도 됩니다.)";
	...
	public ModelAndView cupdate(StoreDTO store) throws IllegalStateException, IOException {
		...
		// 파일 이름 가져오기
		MultipartFile sFile3 = store.getsFile1();
		...
		if (result > 0) {
			...
			mav.setViewName("redirect:sView?storeCode=" + store.getStoreCode());
		} else {
			mav.setViewName("redirect:index");
		}
    ...
    public ModelAndView sModify(StoreDTO store) throws IllegalStateException, IOException {
       ...
       //중복된 파일 이름 방지 UUID
             ...
             // 파일 저장경로 :: 마지막에 "/" 꼭 붙이기 
             String savePath = "C:/Users/1/Desktop/SpringWorkplace/Let's Eat/src/main/webapp/resources/fileUpload/" + sFileName1;
             String savePath2 = "C:/Users/1/Desktop/SpringWorkplace/Let's Eat/src/main/webapp/resources/fileUpload/" + sFileName2;
             String savePath3 = "C:/Users/1/Desktop/SpringWorkplace/Let's Eat/src/main/webapp/resources/fileUpload/" + sFileName3;
             
             // "C:/Users/1/Desktop/SpringWorkplace/Let's Eat/src/main/webapp/resources/fileUpload/"
